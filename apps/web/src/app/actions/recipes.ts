...
async function findOrCreateIngredient(name: string, unit: string, category: string) {
...
export async function createRecipe(formData: FormData) {
  ...
  if (ingredientsJson) {
    try {
      const ingredients: { meat: IngredientInput[]; veggie: IngredientInput[] } = JSON.parse(ingredientsJson)
      ...
      if (meatVersion && ingredients.meat?.length) {
        for (const ing of ingredients.meat) {
          const ingredient = await findOrCreateIngredient(ing.name, ing.unit, ing.category || "Sonstiges")
...
export async function addIngredientToVersion(
  versionId: string,
  recipeId: string,
  ing: { name: string; unit: string; amountPerPerson: number; category: string }
) {
  const ingredient = await findOrCreateIngredient(ing.name, ing.unit, ing.category || "Sonstiges")
  await prisma.recipeIngredient.create({
    data: {
...
export async function removeRecipeIngredient(recipeIngredientId: string, recipeId: string) {
