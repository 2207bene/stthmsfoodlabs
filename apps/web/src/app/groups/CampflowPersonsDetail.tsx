...
import {
  deleteCampflowPerson,
  updateCampflowPerson,
} from "@/app/actions/campflow";
...
function normalizeGenderToOption(
  gender: string | null,
): "" | "male" | "female" | "diverse" {
  if (!gender) return "";
...
export function CampflowPersonsDetail({
    ...
    return [...filtered].sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case "ageGroup":
          ...
          break;
        case "gender":
          cmp = formatGender(a.gender).localeCompare(
            formatGender(b.gender),
            "de",
          );
          break;
        case "diet":
          ...
          break;
        case "intolerances":
          cmp = (a.intolerances ?? "").localeCompare(
            b.intolerances ?? "",
            "de",
          );
          break;
  ...
  function handleAddIntolerance(id: string) {
    ...
    setPersons((prev) =>
      prev.map((p) => (p.id === id ? { ...p, intolerances: updated } : p)),
    );
      ...
      <CardContent className="space-y-5">
        <div className="grid grid-cols-3 gap-6">
          ...
          <div>
            ...
            <div className="space-y-1">
              {(["klein", "mittel", "groß", "unbekannt"] as const).map(
                (group) => {
                        ...
                        >
                          ...
                          <td className="px-3 py-2">
                            <select
                              ...
                              onChange={(e) =>
                                handleGenderChange(p.id, e.target.value)
                              }
                          ...
                          <td className="px-3 py-2">
                            <select
                              ...
                              onChange={(e) =>
                                handleDietChange(p.id, e.target.value)
                              }
                              className="text-xs border rounded px-1 py-0.5 bg-white"
                            >
