...
const CATEGORY_SECTIONS = [
  ...
  { label: "Nachmittag & Snacks", categories: ["Nachmittagssnack", "Beilage", "Dessert"] },
        ...
        <CardContent>
          ...
          <div className="flex flex-wrap gap-2">
            {recipe.versions.some(v => v.type === "MIT_FLEISCH") && (
              <span className="px-2 py-0.5 bg-red-50 text-red-700 rounded text-xs border border-red-100">Fleisch</span>
            )}
            {recipe.versions.some(v => v.type === "VEGETARISCH") && (
              <span className="px-2 py-0.5 bg-green-50 text-green-700 rounded text-xs border border-green-100">Veggie</span>
            )}
            {recipe.tags && recipe.tags.split(",").map(tag => tag.trim() && (
              <span key={tag} className="px-2 py-0.5 bg-secondary text-secondary-foreground rounded text-xs">{tag}</span>
            ))}
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto font-sans">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Rezeptverwaltung</h1>
      ...
      <div className="space-y-10">
        {allSections.map(section => section.recipes.length > 0 && (
          <div key={section.label}>
            <h2 className="text-xl font-semibold mb-4 pb-2 border-b border-border">{section.label}</h2>
