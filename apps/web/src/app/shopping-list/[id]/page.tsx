...
import { toggleItemChecked, updateAmountObtained, confirmPurchase, bookPartialAmount, updateIngredientMetroUrl, regenerateMetroUrlWithAI } from "./actions";
    ...
    <div className="container mx-auto p-4 md:p-6 max-w-3xl">
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Generiert am {format(list.createdAt, "dd. MMMM yyyy, HH:mm", { locale: de })}
          </p>
        </div>
      ...
      <div className="space-y-8 bg-card border rounded-lg p-6 shadow-sm">
        {Object.keys(groupedItems).length === 0 && (
          <p className="text-center text-muted-foreground py-8">Alles ist auf Lager. Keine fehlenden Zutaten!</p>
        )}
        ...
        {Object.entries(groupedItems).map(([category, items]) => (
          <div key={category}>
            ...
            <div className="space-y-4">
              {items.map(item => {
                const stillNeeded = Math.max(0, item.amountTotal - (item.amountObtained ?? 0));
                const fullyObtained = (item.amountObtained ?? 0) >= item.amountTotal;

                return (
                  <div key={item.id} className="border rounded-lg p-3 space-y-2">
                    {/* Toggle row */}
                    <form action={toggleItemChecked} className="flex items-center space-x-3">
                      <input type="hidden" name="itemId" value={item.id} />
                      <input type="hidden" name="checked" value={item.checked ? "false" : "true"} />
                      <button type="submit" className="flex items-center space-x-3 w-full text-left">
                        <Checkbox id={item.id} checked={item.checked} className="pointer-events-none" />
                        <Label htmlFor={item.id} className={`flex-1 cursor-pointer text-base ${item.checked ? "line-through text-muted-foreground" : ""}`}>
                          {item.ingredient.name}
                        </Label>
                        <span className={`font-mono text-sm ${item.checked ? "text-muted-foreground" : "font-medium"}`}>
                    ...
                    <div className="pl-7 space-y-1">
                      <form action={updateAmountObtained} className="flex items-center gap-2">
                        <input type="hidden" name="itemId" value={item.id} />
                        <span className="text-xs text-muted-foreground shrink-0">Besorgt:</span>
                        <input
                          type="number"
                          name="amountObtained"
                          defaultValue={item.amountObtained ?? 0}
                          min={0}
                          step={["kg", "l"].includes(item.ingredient.unit) ? "0.1" : "any"}
                          className="w-20 text-sm border rounded px-2 py-1 text-right"
                        />
                        <span className="text-xs text-muted-foreground">{item.ingredient.unit}</span>
                        <button
                          type="submit"
                      ...
                      {(item.amountObtained ?? 0) > 0 && (
                        <form action={bookPartialAmount} className="flex items-center gap-2">
                          <input type="hidden" name="itemId" value={item.id} />
                          <button
                            type="submit"
                            className="text-xs bg-green-600 text-white px-2 py-1 rounded hover:bg-green-700 transition-colors"
                          >
                            {(item.amountObtained ?? 0).toLocaleString("de-DE")} {item.ingredient.unit} ins Lager einbuchen
                          </button>
                          {!fullyObtained && (
                            <span className="text-xs text-orange-600 font-medium">
                              Noch {stillNeeded.toLocaleString("de-DE")} {item.ingredient.unit} offen
                            </span>
                          )}
                          {fullyObtained && (
                            <span className="text-xs text-green-600 font-medium">Vollständig besorgt</span>
                          )}
                        </form>
                      )}
                    </div>
                    ...
                    <div className="pl-7 pt-2 border-t border-muted/30 flex flex-col sm:flex-row sm:items-center gap-2">
                      ...
                      <div className="flex-1 flex items-center gap-1.5 min-w-0">
                        <form action={updateIngredientMetroUrl} className="flex-1 flex items-center gap-1.5">
                          <input type="hidden" name="ingredientId" value={item.ingredient.id} />
                        ...
                        <form action={regenerateMetroUrlWithAI} className="shrink-0">
                          <input type="hidden" name="ingredientId" value={item.ingredient.id} />
                          <input type="hidden" name="shoppingListId" value={list.id} />
                          <button
                            type="submit"
      ...
      {checkedCount > 0 && list.status !== "abgeschlossen" && (
        <form action={confirmPurchase} className="mt-6">
          ...
          <Button type="submit" className="w-full gap-2 bg-green-600 hover:bg-green-700 text-white">
            ...
            Einkauf bestätigen – {checkedCount} Position{checkedCount !== 1 ? "en" : ""} ins Lager einbuchen
          </Button>
        </form>
      )}
