...
import { addIngredient, bookStock, bookKuchenspende } from "@/app/actions/inventory";
            ...
            <CardContent>
              <div className="flex flex-wrap gap-4">
                {Array.from(unitTotals.entries()).map(([unit, total]) => (
                  <div key={unit} className="bg-white px-4 py-2 rounded-md border shadow-sm">
                    <span className="text-2xl font-bold text-indigo-600">{total.toLocaleString("de-DE")}</span>
                    <span className="text-gray-500 ml-1">{unit}</span>
                  </div>
                ))}
                {unitTotals.size === 0 && (
                  <p className="text-gray-500 text-sm">Noch keine Bestände vorhanden.</p>
                )}
              </div>
            </CardContent>
          </Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {ingredients.filter(i => i.category === category).map(item => {
                    ...
                    return (
                    <div key={item.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 border rounded-lg gap-4">
                      <div className="flex-1">
                        <div className="font-medium flex items-center gap-2">
                          {item.name}
                          {usage > 0 && (
                            <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full ${statusBg} ${statusColor}`} title={`Noch benötigt: ${usage.toLocaleString("de-DE")} ${item.unit}`}>
                              {statusIcon} {statusText}
                            </span>
                          )}
                        </div>
                        <div className="text-sm text-gray-500 mt-1 flex flex-wrap gap-x-4">
                          <span>Bestand: <span className="font-bold text-black">{item.currentStock.toLocaleString("de-DE")} {item.unit}</span></span>
                          {usage > 0 && (
                            <>
                              <span>Benötigt: {usage.toLocaleString("de-DE")} {item.unit}</span>
                              <span className={remaining < 0 ? "text-red-500 font-medium" : ""}>Differenz: {remaining > 0 ? "+" : ""}{remaining.toLocaleString("de-DE")} {item.unit}</span>
                            </>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <form action={async () => {
                          ...
                          <Button variant="outline" size="sm" className="h-8 w-8 p-0" title="1 Einheit verbrauchen">
                        ...
                        <form action={async () => {
                          ...
                          <Button variant="outline" size="sm" className="h-8 w-8 p-0" title="1 Einheit einkaufen">
            ...
            <CardContent>
              <form action={addIngredient} className="space-y-4">
                <div className="space-y-2">
                  <label htmlFor="name" className="text-sm font-medium">Artikelname</label>
                ...
                <div className="space-y-2">
                  ...
                  <select name="category" id="category" className="flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2" required>
                    <option value="Öle & Essige">Öle & Essige</option>
                    <option value="Gewürze & Würzmittel">Gewürze & Würzmittel</option>
                    <option value="Back- & Trockenwaren">Back- & Trockenwaren</option>
                    ...
                    <option value="Reinigung & Sonstiges">Reinigung & Sonstiges</option>
                    <option value="Kuchenspenden">Kuchenspenden</option>
                  </select>
                </div>

                <Button type="submit" className="w-full">Zum Lager hinzufügen</Button>
              </form>
            </CardContent>
          </Card>
          ...
          <Card className="border-amber-200 bg-amber-50/40">
            ...
            <CardContent>
              <form action={bookKuchenspende} className="space-y-4">
                <div className="space-y-2">
                  <label htmlFor="kuchen-name" className="text-sm font-medium">Bezeichnung</label>
                  <Input id="kuchen-name" name="name" required placeholder="z.B. Bienenstich, Obstkuchen…" />
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-2">
                    <label htmlFor="kuchen-amount" className="text-sm font-medium">Menge</label>
                    <Input id="kuchen-amount" name="amount" type="number" step="any" min="0.01" required placeholder="z.B. 1" />
                  </div>
                  <div className="space-y-2">
                    <label htmlFor="kuchen-unit" className="text-sm font-medium">Einheit</label>
                    <select name="unit" id="kuchen-unit" className="flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2" required>
                      <option value="pcs">Stück (pcs)</option>
                      <option value="kg">Kilogramm (kg)</option>
                      <option value="pkg">Packungen (pkg)</option>
                    </select>
                ...
                <div className="space-y-2">
                  <label htmlFor="kuchen-notes" className="text-sm font-medium">Notiz (optional)</label>
                  <Input id="kuchen-notes" name="notes" placeholder="z.B. von Familie Müller" />
                </div>
