...
export async function POST(req: NextRequest) {
  try {
    const { recipeName, category, notes, personsMeat, personsVeggie } =
      await req.json();
...
Mengen als pro-Person-Wert angeben. Wenn kein Fleisch/Veggie-Esser, leeres Array verwenden.`;
    ...
    const message = await anthropic.messages.create({
      ...
      max_tokens: 4096,
      system:
        "Du bist ein Küchen-Assistent für ein Jugend-Sommerlager. Antworte ausschließlich mit gültigem JSON.",
      messages: [{ role: "user", content: userPrompt }],
    });

    if (message.stop_reason === "max_tokens") {
      return NextResponse.json(
        { error: "Antwort zu lang — bitte Zutaten reduzieren." },
        { status: 500 },
      );
    }
