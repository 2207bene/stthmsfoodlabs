...
export async function generateMetroSearchUrl(ingredient: {
  name: string;
  unit: string;
  category: string;
}): Promise<string> {
  ...
  if (!apiKey) {
    console.log(
      `Kein ANTHROPIC_API_KEY gefunden. Nutze Fallback-Link für: ${ingredient.name}`,
    );
    return defaultUrl;
  }
  ...
  try {
    ...
    if (optimizedTerm) {
      ...
      console.log(
        `AI-optimierter Suchbegriff für '${ingredient.name}': '${cleanTerm}'`,
      );
