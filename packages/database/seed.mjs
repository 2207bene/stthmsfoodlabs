...
async function main() {
  ...
  // 1. Frühstück (Dummy) – rein vegetarisch, alle 95 Personen
  const fruehstueck = await prisma.recipe.create({ data: {
    name: 'Frühstück (Standard)',
    category: 'Frühstück',
    tags: 'täglich,frühstück',
    allergens: 'Gluten,Milch',
    notes: 'Tee: 24 l Wasser + 24 Teebeutel/Tag. Morgens warm und pur; nachmittags/abends mit Pulver (2–3 Packungen). Außerdem: Margarine, Marmelade, Nutella, Kaba, Honig, Müsli je nach Verfügbarkeit.',
    versions: { create: [{
      type: 'VEGETARISCH',
      ingredients: { create: [
  ...
  // 2. Abendessen (Dummy) – Fleisch + Veggie
  const abendessen = await prisma.recipe.create({ data: {
    name: 'Abendessen (Standard)',
    category: 'Abendessen',
    tags: 'täglich,abendessen',
    allergens: 'Gluten,Milch',
    notes: 'Vegetarische Brotaufstriche auf das Buffet. Reste vom Mittagessen dazu stellen. Extra: Salate, Paprikacreme etc.',
    versions: { create: [
  ...
  const chili = await prisma.recipe.create({ data: {
    ...
    allergens: 'Gluten,Milch',
    notes: 'Beilage: 36 Aufbackbaguette (groß) + 2× glutenfreies Baguette. Wassermelone: 3 Stück.',
    versions: { create: [
      { type: 'MIT_FLEISCH', ingredients: { create: [
  ...
  const kaesespaetzle = await prisma.recipe.create({ data: {
    ...
    notes: 'Für Gluten-Allergiker: glutenfreie Nudeln überbacken (noch klären). Abends: Paprikacreme (1,7 kg Frischkäse, 675 g Crème fraîche, 6 rote Paprika, 350 g Tomatenmark, 3–4 Zwiebeln).',
    versions: { create: [{
      type: 'VEGETARISCH',
      ingredients: { create: [
        // Käsespätzle
  ...
  // Veggie-Version: gleiche Zutaten wie Fleisch, nur ohne Putenbrust
  const curry = await prisma.recipe.create({ data: {
    name: 'Curry mit Reis und Mousse au Chocolat',
    category: 'Hauptgericht',
    tags: 'asiatisch,scharf',
    allergens: 'Milch',
    notes: 'Soßenbinder (Mondamin hell) und Gemüsebrühe vor Ort prüfen! 1–2 kg Cashewkerne optional. Mousse au Chocolat: 2 Metro-Packungen (je 1 kg Pulver) + 5 l Milch = 116 Portionen. Abends: Tomaten-Mozzarella (50 Tomaten, 2 kg Mozzarellarolle, Basilikum-Topf).',
    versions: { create: [
      { type: 'MIT_FLEISCH', ingredients: { create: [
      ...
      { type: 'VEGETARISCH', ingredients: { create: [
        // Gleiche Zutaten ohne Putenbrust
        { ingredientId: I.kokosmilch.id,         amountPerPerson: r(5.94  / P_MEAT), unit: 'l'     },
        { ingredientId: I.currypaste.id,         amountPerPerson: r(2.38  / P_MEAT), unit: 'Glas'  },
        { ingredientId: I.joghurt.id,            amountPerPerson: r(4.75  / P_MEAT), unit: 'l'     },
        { ingredientId: I.tomatenmark.id,        amountPerPerson: r(0.475 / P_MEAT), unit: 'kg'    },
  ...
  const gefuelltePaprika = await prisma.recipe.create({ data: {
    ...
    notes: 'Morgens: Rührei extra (70 Eier). Beilage: 3 kg Reis. Brühe nach Bedarf ca. 2–3 l. 4–5 Packungen Sahnesteif für Apfelschaum. Besser Haselnusskrokant statt Mandeln.',
    versions: { create: [{
      type: 'VEGETARISCH',
      ingredients: { create: [
  ...
  const knoedel = await prisma.recipe.create({ data: {
    ...
    allergens: 'Gluten,Milch,Ei',
    notes: 'Glutenfreie Alternative für Knödel klären (6 glutenfreie Knödel). Abends: Currywurst (70 Bockwürste + 25 vegetarische Würste + 90 Baguette-Brötchen + 2× eigene Brötchen).',
    versions: { create: [{
      type: 'VEGETARISCH',
      ingredients: { create: [
        // Knödelmasse
        ...
        // Sahne gesamt (Pilzrahmsoße 4,75 l + Solero 1,13 l)
  ...
  // Hackfleischsoße: Originalmengen für 30 P. → hochgerechnet auf 70 P.
  const SCALE = 70 / 30
  const spaghetti = await prisma.recipe.create({ data: {
    ...
    allergens: 'Gluten,Milch',
    notes: 'SONDERPROGRAMM! Kandierte Tomaten (2 kg Kirschtomaten + Olivenöl + Zucker + ital. Kräuter). 2 Töpfe Basilikum + 1 kg Parmesan. Glutenfreie Spaghetti (1 Packung). Abends: Couscous-Salat (1,78 kg Couscous, 5 Schlangengurken, 9 Tomaten, 7 gelbe Paprika, 2,5 Bund Frühlingszwiebeln).',
    versions: { create: [
      { type: 'MIT_FLEISCH', ingredients: { create: [
        ...
        { ingredientId: I.passTomaten.id,    amountPerPerson: r(1.13 * SCALE          / P_MEAT), unit: 'l'     },
        { ingredientId: I.stueckTomaten.id,  amountPerPerson: r(1.13 * SCALE          / P_MEAT), unit: 'l'     },
        // Tomatensoße (für alle)
  ...
  console.log('✅ All 10 recipes created')
  ...
  // Mittag-Zuordnung: [recipe, hasMeatVersion]
  const mittagMap = [
    [chili,            true ],  // Sa 02.08.
    [kaesespaetzle,    false],  // So 03.08. – 100% veg → alle 95 als veggie
    [curry,            true ],  // Mo 04.08.
    [gefuelltePaprika, false],  // Di 05.08. – 100% veg
    [knoedel,          false],  // Mi 06.08. – 100% veg
  ...
  console.log(`   Rezepte:         10`)
  console.log(`   Speiseplanzeilen: ${campDates.length * 3}  (Frühstück + Mittag + Abend × 8 Tage)`)
}
